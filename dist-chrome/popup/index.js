...
(() => {
  ...
  var require_chrome_promise = __commonJS({
    "node_modules/chrome-promise/chrome-promise.js"(exports, module) {
      (function(root2, factory) {
        if (typeof exports === "object") {
          module.exports = factory(this || root2);
        } else if (typeof define === "function" && define.amd) {
          define([], factory.bind(null, this || root2));
        } else {
          root2.ChromePromise = factory(root2);
          var script = document.currentScript;
          if (script) {
            var name = script.dataset.instance;
            if (name) {
              root2[name] = new root2.ChromePromise();
            }
          }
        }
      })(typeof self !== "undefined" ? self : exports, function(root2) {
        "use strict";
        var slice = Array.prototype.slice, hasOwnProperty = Object.prototype.hasOwnProperty;
        ChromePromise.default = ChromePromise;
        return ChromePromise;
        function ChromePromise(options) {
          options = options || {};
          var chrome2 = options.chrome || root2.chrome;
          var Promise2 = options.Promise || root2.Promise;
          var runtime = chrome2.runtime;
          var self2 = this;
          if (!self2)
            throw new Error("ChromePromise must be called with new keyword");
          fillProperties(chrome2, self2);
          if (chrome2.permissions) {
            chrome2.permissions.onAdded.addListener(permissionsAddedListener);
          }
          ...
          function permissionsAddedListener(perms) {
            if (perms.permissions && perms.permissions.length) {
              ...
              perms.permissions.forEach(function(permission) {
                ...
                if (api in chrome2) {
                  approvedPerms[api] = chrome2[api];
                }
              });
              fillProperties(approvedPerms, self2);
            }
          }
        }
      });
  ...
  var require_chrome_promise2 = __commonJS({
    "node_modules/chrome-promise/index.js"(exports, module) {
      var ChromePromise = require_chrome_promise();
      var chromep2 = new ChromePromise();
      chromep2.default = chromep2;
      module.exports = chromep2;
    }
  });
  ...
  // node_modules/@extend-chrome/storage/lib/index-esm.js
  var import_chrome_promise = __toESM(require_chrome_promise2());
  var invalidSetterReturn = (r) => {
    if (Array.isArray(r)) {
  ...
  var getStorageArea = (area) => {
    switch (area) {
      case "local":
        return import_chrome_promise.default.storage.local;
      case "sync":
        return import_chrome_promise.default.storage.sync;
      case "managed":
        return import_chrome_promise.default.storage.managed;
      default:
  ...
  function getBucket(bucketName, areaName) {
    ...
    const storage2 = getStorageArea(_areaName);
    ...
    const getKeys = async () => {
      const result = await storage2.get(keys);
      return result[keys] || [];
    };
    const setKeys = (_keys) => {
      return storage2.set({ [keys]: _keys });
    };
    let promise = null;
    async function coreGet(x) {
      ...
      if (typeof x === "undefined" || x === null) {
        ...
        if (!keys2.length) {
          ...
          const getter = pfxAry(keys2);
          const result = await storage2.get(getter);
        ...
        if (promise === null) {
          promise = coreGet().then((prev) => {
            try {
              ...
              return storage2.set(pfxNext).then(() => next);
            } finally {
    ...
    const remove = (arg) => {
      ...
      return storage2.remove(pfxAry(query)).then(getKeys).then(_setKeys);
    };
    ...
    const changeStream = nativeChange$.pipe(filter(([changes, area]) => {
      return area === areaName && Object.keys(changes).some((k) => k.startsWith(prefix));
    }), map(([changes]) => {
      const bucketChanges = Object.keys(changes).filter((k) => k.startsWith(prefix) && k !== keys);
      return bucketChanges.length ? bucketChanges.reduce((r, k) => Object.assign(Object.assign({}, r), { [unpfx(k)]: changes[k] }), {}) : void 0;
    }), filter(isNonNull));
    return {
      set,
      get,
  ...
  var Popup = () => {
    const saveCourse = () => {
      const courses = document.querySelectorAll(".contentnode ul li a");
      courses.forEach((course) => {
        const courseName = course.textContent;
        const courseUrl = course.getAttribute("href");
        const courseId = courseUrl == null ? void 0 : courseUrl.split("id=")[1].split("&")[0];
        if (courseName && courseId) {
          storage.local.set({ courseId, courseName });
        }
      });
    };
    return /* @__PURE__ */ import_react.default.createElement("button", { onClick: saveCourse }, "\u8AAD\u307F\u8FBC\u3080");
  };

  // popup/index.tsx
  var container = document.getElementById("popup");
  var root = (0, import_client.createRoot)(container);
  root.render(/* @__PURE__ */ import_react2.default.createElement(Popup, null));
})();
/*!
 * chrome-promise
